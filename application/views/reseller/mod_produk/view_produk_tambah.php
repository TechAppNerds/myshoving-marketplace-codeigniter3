            ...
            echo "<div class='col-md-12'>
                    <table class='table table-condensed table-bordered'>
                      <tbody>
                        ...
                        <tr><th scope='row'>Satuan</th><td><input type='text' class='form-control' min='1' name='c'></td></tr>
                        <tr><th scope='row'>Berat / Gram</th><td><input type='number' class='form-control' min='1' name='berat' value='0'></td></tr>
                        <tr><th scope='row'>Harga Modal</th><td><input type='number' class='form-control' min='1' name='d' value='0'></td></tr>
                        <input type='hidden' class='form-control' name='e' value='0'>
                        <tr><th scope='row'>Harga Jual</th><td><input type='number' class='form-control' min='1' name='f' value='0'></td></tr>
                        <tr><th scope='row'>Diskon</th><td><input type='number' class='form-control' min='0' name='diskon' value='0' onkeypress='return event.charCode >= 48 && event.charCode <= 57'></td></tr>
                        <tr><th scope='row'>Stok Awal</th><td><input type='number' class='form-control' min='1' name='stok' value='0'></td></tr>
                        <tr><th scope='row'>Keterangan</th><td><textarea class='textarea form-control' name='ff' style='height:180px'></textarea></td></tr>
                        <tr><th scope='row'>Foto Produk</th><td><input type='file' id='fileupload' class='form-control' onchange='filePreview(fileupload)' name='userfile[]' multiple>Multiple Upload, Allowed File : .gif, jpg, png <br/><br/><div id='dvPreview'></div> </td></tr>
                      </tbody>
                    </table>
                  </div>
                </div>
  ...
  function filePreview(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            ...
            $('#dvPreview').after('<img src="'+e.target.result+'" width="175" height="175"/>');
        };
        reader.readAsDataURL(input.files[0]);
    }
}
  $("input[name='diskon']").on('input propertychange', function(){
    let cek=0, lengthval=$(this).val().length;
    if (lengthval < 1) $(this).val(0);
    else if ($(this).val()<100) {
      while(cek < lengthval-1) {
        if ($(this).val().substring(cek,cek+1) < 1) $(this).val($(this).val()%(Math.pow(10,lengthval-1))); cek++;
      }
    }
